<div class="paintbbs_memo">
  <div>基本の動作(恐らくこれだけは覚えておいた方が良い機能)</div>
            <div> &lt;基本&gt;<br>
              ...
              右クリックで逆周り。パレットの色,マスクの色,一字保存ツールに現在の状態を登録は全て右クリックです。<br>
              逆にクリックでパレットの色と一時保存ツールに保存しておいた状態を取り出せます。<br>
              <br>
              &lt;キャンバス部分&gt;<br>
              右クリックで色をスポイトします<br>
              ベジエやコピー等の処理の途中で右クリックを押すとリセットします。 </div>
          <br>
          ...
          <div> &lt;ツールバー&gt;<br>
            ...
            続けます。そうする事によりレイヤ間の移動が可能になります。<br>
            </div>
          <br>
            <div>ツールバーのボタンと特殊な機能の簡単な説明</div>
          <div>
              <ul type="circle">
                <li>ペン先(通常ペン,水彩ペン,テキスト)<br>
                  メインのフリーライン系のペンとテキスト<br>
                  <br>
                </li>
                <li>ペン先2(トーン,ぼかし,他)<br>
                  特殊な効果を出すフリーライン系のペン<br>
                  <br>
                </li>
                <li>図形(円や長方形)<br>
                  長方形や円等の図形<br>
                ...
                <li>特殊(コピーやレイヤー結合,反転等)<br>
                  コピーは一度選択後、ドラッグして移動、コピーさせるツールです。<br>
                  <br>
                </li>
                <li>マスクモード指定(通常,マスク,逆マスク）<br>
                  マスクで登録されている色を描写不可にします。逆マスクはその逆。<br>
                  通常でマスク無し。また右クリックでマスクカラーの変更が可能。<br>
                  <br>
                </li>
                <li>消しゴム(消しペン,消し四角,全消し)<br>
                  透過レイヤー上を白で塗り潰した場合、下のレイヤーが見えなくなりますので<br>
                  上位レイヤーの線を消す時にはこのツールで消す様にして下さい。全消しはすべてを透過ピクセル化させるツールです。<br>
                  全けしを利用する場合はこのツールを選択後キャンバスをクリックでOK。<br>
                  消しゴムは独立した線の幅を持っています。<br>
                  <br>
                </li>
                <li>描写方法の指定。(手書き,直線,ベジエ曲線)<br>
                  ペン先,描写機能指定ではありません。<br>
                  また適用されるのはフリーライン系のツールのみです。<br>
                  <br>
                </li>
                <li>カラーパレット郡<br>
                  クリックで色取得。右クリックで色の登録。Shift+クリックでデフォルト値。<br>
                  <br>
                </li>
                <li>RGBバーとalphaバー<br>
                  細かい色の変更と透過度の変更。Rは赤,Gは緑,Bは青,Aは透過度を指します。<br>
                  トーンはAlphaバーで値を変更する事で密度の変更が可能です。<br>
                  <br>
                </li>
                <li>線幅変更ツール<br>
                  水彩ペンを選択時に線幅を変更した時、デフォルトの値がalpha値に代入されます。<br>
                  <br>
                </li>
                <li>線一時保存ツール<br>
                  クリックでデータ取得。右クリックでデータの登録。(マスク値は登録しません)<br>
                  <br>
                </li>
                <li>レイヤーツール<br>
                  ...
                  クリックでレイヤー入れ替え。右クリックで選択されているレイヤの表示、非表示切り替え。<br>
                  <br>
                </li>
              </ul>
          </div>
          投稿に関して：
          <div> 絵が完成したら投稿ボタンで投稿します。<br>
            絵の投稿が成功した場合は指定されたURLへジャンプします。<br>
            失敗した場合は失敗したと報告するのみでどこにも飛びません。<br>
            単に重かっただけである場合、少し間を置いた後、再度投稿を試みて下さい。<br>
            この際二重で投稿される場合があるかもしれませんがそれは<br>
            WebサーバーかCGI側の処理ですのであしからず。 </div>
</div>
